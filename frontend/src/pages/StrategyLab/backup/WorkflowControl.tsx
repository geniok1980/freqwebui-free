...
function StrategySelector({ strategies, selected, onSelect }) {
  ...
  const selectedName = strategies.find(s => s.name === selected)?.name || 'Select a strategy...';
        ...
        <div className="absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg max-h-80 overflow-hidden">
          <div className="p-2 border-b border-gray-200 dark:border-gray-700">
            <input
              ...
              autoFocus
            />
            <p className="text-xs text-gray-500 mt-1">{filtered.length} of {strategies.length} shown</p>
          </div>
...
export function WorkflowControl() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/strategy-lab/strategies`, { headers: { 'Authorization': 'Bearer ' + token }})
      .then(r => r.json())
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Strategy to Optimize ({strategies.length} available)</label>
