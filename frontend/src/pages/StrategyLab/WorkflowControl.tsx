...
function StrategySelector({
  ...
  const validStrategies = strategies.filter((s: Strategy) => s && s.name);

  const filtered = validStrategies.filter((s: Strategy) =>
    s.name.toLowerCase().includes(search.toLowerCase())
  );

  const selectedName =
    validStrategies.find((s: Strategy) => s.name === selected)?.name ||
    'Выберите стратегию...';
        ...
        <div className="absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg max-h-80 overflow-hidden">
          <div className="p-2 border-b border-gray-200 dark:border-gray-700">
            ...
            <p className="text-xs text-gray-500 mt-1">
              {filtered.length} {t('common.of')} {validStrategies.length} {t('common.shown')}
            </p>
          </div>
          <div className="overflow-y-auto max-h-64">
            {filtered.map((s: Strategy, idx: number) => (
              <div
                key={`${s.name}-${idx}`}
                onClick={() => {
                  onSelect(s.name);
...
export function WorkflowControl() {
  ...
  const startWorkflow = async () => {
    if (!selectedStrategy) {
      setMessage('Сначала выберите стратегию!');
      return;
    }
    setLoading(true);
    setMessage('Запуск...');
    try {
      ...
      if (res.ok) {
        ...
        setMessage('Workflow запущен: ' + data.message);
      } else {
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
            ⚙️ {t('strategyLab.workflowControl')}
          </h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            Automate: stop → download → backtest → hyperopt → deploy
          </p>
        </div>
        <Link to="/strategy-lab" className="text-blue-600 dark:text-blue-400">
          ← {t('common.back')}
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
          ...
          <select
            value={selectedBot}
            onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setSelectedBot(e.target.value)}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
            Стратегия для оптимизации{' '}
            <span className="text-blue-600 font-bold">({strategies.length} загружено)</span>
          </label>
          <StrategySelector
          ...
          {strategies.length === 0 && (
            <p className="text-sm text-red-500 mt-2">
              ⚠️ Стратегии не загружены. Проверьте консоль на ошибки.
            </p>
          )}
        </div>
      </div>
      ...
      {selectedStrategy && (
        <>
          ...
          <div
            className={`bg-white dark:bg-gray-800 rounded-xl border-l-4 p-6 mb-6 ${
            ...
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-semibold text-gray-900 dark:text-white">
                  Статус:{' '}
                  <span className={isRunning ? 'text-orange-600' : 'text-green-600'}>
                    {workflowStatus?.status || 'idle'}
                  </span>
                </h3>
                {workflowStatus?.current_step && (
                  <p className="text-sm text-gray-500 mt-1">
                    Текущий шаг: {workflowStatus.current_step}
                  </p>
                )}
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                    setConfig({ ...config, max_drawdown_threshold: parseFloat(e.target.value) })
                  }
                  disabled={isRunning}
                  className="w-full px-3 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                />
              ...
              <div className="flex items-end">
                <label className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      setConfig({ ...config, auto_promote: e.target.checked })
                    }
                    disabled={isRunning}
                    className="w-5 h-5"
                  />
                  <span className="text-sm text-gray-700 dark:text-gray-300">Авто-промоут</span>
                </label>
              </div>
            </div>
          </div>
