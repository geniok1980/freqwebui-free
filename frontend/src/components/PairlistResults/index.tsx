        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-4 gap-4">
            <div className="bg-[#161b22] border border-[#30363d] rounded-lg p-4">
              <p className="text-gray-400 text-sm">{t('pairlist.totalPairs')}</p>
              <p className="text-2xl font-bold text-white">{resultData.summary?.total_pairs}</p>
            </div>
            <div className="bg-[#161b22] border border-[#30363d] rounded-lg p-4">
              <p className="text-gray-400 text-sm">{t('pairlist.bestProfit')}</p>
              <p className="text-2xl font-bold text-green-400">
                {resultData.summary?.best_profit?.toFixed(2)}%
              </p>
            </div>
            <div className="bg-[#161b22] border border-[#30363d] rounded-lg p-4">
              <p className="text-gray-400 text-sm">{t('pairlist.bestSharpe')}</p>
              <p className="text-2xl font-bold text-blue-400">
                {resultData.summary?.best_sharpe?.toFixed(2)}
              </p>
            </div>
            <div className="bg-[#161b22] border border-[#30363d] rounded-lg p-4">
              <p className="text-gray-400 text-sm">{t('pairlist.avgWinRate')}</p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-[#0f1419]">
                  <tr>
                    <th className="px-4 py-2 text-left text-sm text-gray-400">{t('pairlist.rank')}</th>
                    ...
                    <th className="px-4 py-2 text-right text-sm text-gray-400">{t('pairlist.winRate')}</th>
                    <th className="px-4 py-2 text-right text-sm text-gray-400">Sharpe</th>
                    <th className="px-4 py-2 text-right text-sm text-gray-400">{t('pairlist.trades')}</th>
                    <th className="px-4 py-2 text-right text-sm text-gray-400">{t('pairlist.score')}</th>
                  </tr>
                </thead>
                <tbody>
