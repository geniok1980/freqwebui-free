...
export function UnifiedSettings() {
  ...
  const saveMutation = useMutation({
    ...
    onError: () => setMessage(t('settings.saveFailed'))
  });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={handleSave} disabled={saveMutation.isPending} className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg">
          <Save className="w-4 h-4" /> {saveMutation.isPending ? t('settings.saving') : t('settings.saveAll')}
        </button>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div>
            <label className="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              <Clock className="w-4 h-4" /> {t('settings.refreshInterval')}
            </label>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{t('settings.dashboardRefreshInterval')}</p>
          </div>
          
          {/* Theme */}
          <div>
            <label className="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              {settings.theme === 'light' ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />} {t('settings.theme')}
            </label>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
          <Server className="w-5 h-5 text-blue-500" /> {t('settings.discoverySettings')}
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label className="block text-sm text-gray-700 dark:text-gray-300 mb-1">{t('settings.hostIp')}</label>
          ...
          <div>
            <label className="block text-sm text-gray-700 dark:text-gray-300 mb-1 flex items-center gap-1"><Lock className="w-3 h-3" /> {t('settings.apiLogin')}</label>
          ...
          <div>
            <label className="block text-sm text-gray-700 dark:text-gray-300 mb-1 flex items-center gap-1"><Lock className="w-3 h-3" /> {t('settings.apiPassword')}</label>
