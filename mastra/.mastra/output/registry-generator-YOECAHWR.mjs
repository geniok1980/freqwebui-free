import { s as shouldEnableGateway, o as getGatewayId } from './mastra.mjs';
...
async function atomicWriteFile(filePath, content, encoding = "utf-8") {
  ...
  try {
    await fs__default.writeFile(tempPath, content, encoding);
    await fs__default.rename(tempPath, filePath);
  } catch (error) {
    try {
...
async function writeRegistryFiles(jsonPath, typesPath, providers, models, attachmentCapabilities, temperatureCapabilities) {
  ...
  await fs__default.mkdir(jsonDir, { recursive: true });
  await fs__default.mkdir(typesDir, { recursive: true });
  const registryData = {
    providers,
    models,
    version: "1.0.0"
  ...
  if (hasCapabilities) {
    const capDir = nodePath__default.join(jsonDir, "capabilities");
    await fs__default.mkdir(capDir, { recursive: true });
    try {
      const existing = await fs__default.readdir(capDir);
      for (const file of existing) {
        if (file.endsWith(".json")) {
          await fs__default.unlink(nodePath__default.join(capDir, file));
        }
      }
    } catch {
    }
    ...
    for (const provider of allProviderIds) {
      ...
      const providerFile = nodePath__default.join(capDir, `${provider}.json`);
